...
// Handle saving edits
$success_message = "";
if (isset($_POST['save_marks']) && !empty($students)) {
    foreach ($_POST['marks'] as $student_id => $data) {
        $test = mysqli_real_escape_string($conn, $data['test'] ?? 0);
        $assignment = mysqli_real_escape_string($conn, $data['assignment'] ?? 0);
        $exam = mysqli_real_escape_string($conn, $data['exam'] ?? 0);

        $check = mysqli_query($conn, "SELECT * FROM assessments WHERE student_id='$student_id' AND subject_id='$subject_id'");

        if (mysqli_num_rows($check) > 0) {
            mysqli_query($conn, "
                UPDATE assessments 
                SET test_mark='$test', assignment_mark='$assignment', exam_mark='$exam' 
                WHERE student_id='$student_id' AND subject_id='$subject_id'
            ");
        } else {
            mysqli_query($conn, "
                INSERT INTO assessments (student_id, subject_id, test_mark, assignment_mark, exam_mark)
                VALUES ('$student_id','$subject_id','$test','$assignment','$exam')
            ");
        }
    }
...
<title>Edit Marks | Teacher Portal</title>
<style>
/* ===============================
   ROOT VARIABLES
================================*/
...
<div class="sidebar">
    ...
    <div class="main">
        ...
        <?php if ($success_message): ?>
            ...
            <script>
                setTimeout(() => {
                    window.location.href = 'edit_marks.php?class_id=<?= htmlspecialchars($class_id) ?>&subject_id=<?= htmlspecialchars($subject_id) ?>';
                }, 1500);
            </script>
        <?php endif; ?>
        ...
        <div class="section">
            <form method="POST">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><input type="number" name="marks[<?= $student['student_id'] ?>][test]" min="0" max="20" value="<?= $student['test_mark'] ?? '' ?>" required></td>
                            <td><input type="number" name="marks[<?= $student['student_id'] ?>][assignment]" min="0" max="20" value="<?= $student['assignment_mark'] ?? '' ?>" required></td>
                            <td><input type="number" name="marks[<?= $student['student_id'] ?>][exam]" min="0" max="60" value="<?= $student['exam_mark'] ?? '' ?>" required></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                <div class="action-area">
                    <button type="submit" name="save_marks" class="btn">Save Changes</button>
                </div>
            </form>
        </div>
        <?php elseif ($class_id && $subject_id): ?>
            <div class="section">
                <p style="color:#e74c3c; text-align:center;">No students found in this class.</p>
            </div>
        <?php endif; ?>
